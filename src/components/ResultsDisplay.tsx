      ...
      <div className="space-y-4">
          ...
          <>
            ...
            {result.upload_stats && (
              <div className="mt-6 p-4 bg-blue-500/20 border border-blue-400/30 rounded-lg">
                <h3 className="text-blue-200 font-medium mb-2">Upload Statistics:</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    <span className="text-blue-100">Successful:</span>
                    <span className="text-white font-bold ml-2">{result.upload_stats.successful_uploads}</span>
                  </div>
                  <div>
                    <span className="text-blue-100">Failed:</span>
            ...
            <div className="mt-6 p-4 bg-green-500/20 border border-green-400/30 rounded-lg">
              <p className="text-green-200 font-medium">✅ Success! Your RAG system is ready:</p>
              <ul className="text-green-100 mt-2 space-y-1 text-sm">
            ...
            <div className="mt-6 p-4 bg-purple-500/20 border border-purple-400/30 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-purple-200 font-medium">📋 Database Schema (if needed):</h3>
                <button
        ...
        {result.processing_errors && result.processing_errors.length > 0 && (
          <div className="mt-4 p-4 bg-yellow-500/20 border border-yellow-400/30 rounded-lg">
            <p className="text-yellow-200 font-medium">⚠️ Processing Warnings:</p>
            <ul className="text-yellow-100 mt-2 space-y-1 text-sm">
        ...
        {result.error && (
          <div className="mt-4 p-4 bg-red-500/20 border border-red-400/30 rounded-lg">
            ...
            <p className="text-red-100 mt-1 text-sm">{result.error}</p>
          </div>
        )}

        {!result.success && (
          <div className="mt-6 p-4 bg-blue-500/20 border border-blue-400/30 rounded-lg">
            <p className="text-blue-200 font-medium">💡 Troubleshooting Tips:</p>
            <ul className="text-blue-100 mt-2 space-y-1 text-sm">
